    ...
    <div className="min-h-screen bg-stone-950 text-stone-200">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-serif text-amber-200 flex-1">{isNew ? 'New Character' : 'Edit Character'}</h1>
          <button onClick={generate} disabled={generating} className="flex items-center gap-2 px-3 py-2 bg-purple-900/40 hover:bg-purple-800/50 border border-purple-700/40 text-purple-200 rounded-lg text-sm">
            <Sparkles className="w-4 h-4" /> {generating ? 'Generating...' : 'AI Generate'}
          </button>
          <button onClick={save} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-amber-700 hover:bg-amber-600 text-amber-50 rounded-lg text-sm font-semibold">
