  ...
  const start = async () => {
    ...
    try {
      ...
      const conv = await base44.agents.createConversation({
        agent_name: 'story_recap',
        metadata: { name: camp ? `Recap: ${camp.name}` : 'Story Recap' }
      });
      setConversation(conv);
      setMessages(conv.messages || []);
      const seed = camp
        ? `Recap my campaign "${camp.name}" (campaign_id: ${camp.id}). Read the campaign, its message logs, and any SavedStory for it, then offer me recap styles and deliver an immersive recap.`
        : `I'd like a recap of my adventure. Ask which campaign to recap, then read its logs and craft an immersive recap.`;
      setBusy(true);
  ...
  if (!conversation) {
    return (
      <div className="min-h-screen bg-background text-foreground">
        <ScreenHeader title="Story Recap" icon={ScrollText} />
        <div className="max-w-2xl mx-auto px-4 py-6">
          <div className="bg-gradient-to-br from-card/80 to-background/80 border border-amber-900/30 rounded-xl p-6 mb-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h2 className="text-lg font-serif text-amber-200">The Chronicler</h2>
                <p className="text-sm text-muted-foreground">Immersive recaps drawn from your campaign logs.</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground">
              The Chronicler reads your campaign, its message logs, and saved stories, then weaves a vivid recap — a cold-open "Previously On…", a full chronicle, the last session, a character-focused retelling, or a cliffhanger teaser.
            </p>
          </div>
          ...
          <div className="bg-card/60 border border-border rounded-xl p-4 mb-4">
            ...
            {campaignId && (
              <p className="text-xs text-muted-foreground mt-2">
                {campaigns.find(c => c.id === campaignId)?.name || '—'} · {campaigns.find(c => c.id === campaignId)?.status || '—'}
              </p>
            )}
            {!campaignId && <p className="text-xs text-muted-foreground mt-2">Pick a campaign, or let the Chronicler ask you.</p>}
          </div>
          ...
          >
            {starting ? 'Opening the chronicle…' : 'Begin Recap'}
          </button>
        </div>
      </div>
    );
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 max-w-3xl mx-auto w-full overscroll-none">
        ...
        {busy && (
          <div className="flex items-center gap-2 text-muted-foreground text-sm ml-2">
            ...
            <span>The Chronicler is recounting…</span>
          </div>
        )}
        <div ref={endRef} />
      </div>
      <div className="border-t border-border p-4 bg-card/50 safe-bottom">
        <div className="max-w-3xl mx-auto flex gap-2">
          <textarea
            ...
            placeholder="Reply to the Chronicler…"
            rows={1}
