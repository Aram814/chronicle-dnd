...
import { ChevronDown, Heart, Frown, Meh, Smile, Angry } from 'lucide-react';
  ...
  return (
    <div className="space-y-2">
      ...
      {npcs.map(n => {
        ...
        const half = Math.abs(disp) / 2; // bar spans -100..100 over 100% width
        ...
        return (
          <div key={n.id} className="bg-background/50 border border-border rounded p-2">
            ...
            {/* Disposition meter: bar grows left (negative) or right (positive) from center */}
            ...
            {n.relationship && <p className="text-xs text-amber-600 mt-1">{n.relationship}</p>}
